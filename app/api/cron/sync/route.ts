 ...
 * puesta la corrida que sostiene el historial de los vinculados.
 */
...
async function runSync(req: Request) {
  ...
  try {
    ...
    /**
     * Va acá arriba junto con el baseline, y ANTES del corte por "no hay
     * trackeados", por el mismo motivo: la foto diaria es del ladder entero.
     * Con esto abajo, un sitio sin nadie vinculado nunca acumularía el archivo
     * que justamente existe para los no vinculados.
     */
    const dailySaved = await saveDaily(board, now);
